...
class SrcType(object):
    ...
    def detect1(cls, src):
        ...
        for i in _punk_list:
            src = src.replace(i, i+'@#@punc@#@')
        parts_list = [sent.strip()
                      for sent in src.split('@#@punc@#@') if len(sent) > 0]
        sent_list = []
        sent = []
        del src
        ...
        for index, parts in enumerate(parts_list):
            ...
            if table['length'] > 300:
                ...
                seprater_small = cls._detect(
                    ''.join(sent).strip(), cls.punk_small_list)  # 按小标点切分
                if seprater_small[0] == 'doc':
                    # 是否可用小标点分割
                    sent_list.extend(seprater_small[1])
                else:
                    # 按长度切割
                    sent_list.extend([''.join(sent).strip()[i:i+chunk_size]
                                     for i in range(0, len(''.join(sent).strip()), chunk_size)])
                sent.clear()
                table = {'patch_flag': False, 'fragments': 0, 'length': 0}
                ...
                if not table['patch_flag']:
                    ...
                    try:
                        if sent[-2][-1] in cls.punk_list:
                            ...
                            sent.clear()
                            table = {'patch_flag': False,
                                     'fragments': 0, 'length': 0}
                    except IndexError:
                        pass
    ...
    def detect2(cls, src):
        ...
        for index, _char in enumerate(src):
            ...
            if len(''.join(sent).strip()) > 300:
                # 是否超长没有标点
                seprater_small = cls._detect(
                    ''.join(sent).strip(), cls.punk_small_list)
                if seprater_small[0] == 'doc':
                    sent_list.extend(seprater_small[1])
                else:
                    sent_list.extend([''.join(sent).strip()[i:i+150]
                                     for i in range(0, len(''.join(sent).strip()), 150)])
                sent.clear()
                patch_flag = False
